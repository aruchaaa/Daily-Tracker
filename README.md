...
`daily-tracker-v65`. The app also auto-reloads itself once when a newer
...
## What's new (cache v65)

- **Settings spacing tidy-up**: the Push Reminders section now has equal
  breathing room above and below its buttons (12px both sides), so the
  toggle/Test buttons no longer sit flush against the guide text beneath.
...
- **Three daily nudges a day**: alongside the per-task reminders, the app
  now sends three generic reminders (morning, afternoon, evening) at
  randomized-but-fixed times per day, so you're prodded even on days with
  no scheduled tasks. The exact minute is randomized per date (morning
  07–09, afternoon 12–15, evening 19–21) and is not user-editable — only
  individually scheduled tasks carry a settable time. A nudge is skipped
  once every task that day is done (and on days with no tasks at all), and
  it fires both as a background push (when push is enabled) and in-app while
  the tab is open (when Reminders is enabled).
...
- **Report controls tidied**: the month dropdown now shares the exact 40px
  height of the Generate button beside it, so the row reads symmetrical.
  The picker arrow is sized, right-aligned, and theme-aware (light glyph on
  the dark Stat Sheet / Neon themes, dark ink on Adventurer's Log) so it
  stays crisp on every theme instead of the default dark-on-dark smudge.
- **Export labels reworded** (Report): the per-month button now reads
  "Export This Month (CSV)" and the all-history one "Export All Time (CSV)"
  (Indonesian: "Export Bulan Ini (CSV)" / "Export Semua Waktu (CSV)");
  button text is a little smaller so both labels fit the row on one line.
...
- **Security headers**: a strict Content-Security-Policy is now enforced —
  ...
  inline scripts anywhere in the app, so the policy is tight
  (`script-src 'self'`, `object-src 'none'`, `base-uri 'none'`, …) and
  clears Lighthouse's Best Practices. `style-src` keeps `'unsafe-inline'`
  because the app sets inline styles at runtime (theme accent +
  progress widths).

## What's new (cache v56)

- **Weekly repeat schedule** (Tasks tab): every task can now repeat on
  chosen days of the week (Mon–Sun chips; all selected = every day, the old
  behavior). Home, History, the nightly reminders, and the monthly report
  all honor it — a Mon/Wed/Fri task only counts (and only shows on) those
  weekdays, and per-task detail shows its recurring days.
- **Deactivation tracking** ("archive" a task): tasks toggled off now stop
  appearing in History and the monthly report after the day they were
  deactivated, instead of lingering as permanently "not done". Reactivating
  restores them. A deactivated task still shows on its final day.
- **Home quick wins**: an always-visible "Yesterday" recap pill (done/total
  + EXP, or a "lets get moving" nudge when nothing was done) and a
  current-streak chip when you're on a roll; afternoon greeting icon fixed.
- **Weekly breakdown on Report**: each month now shows a per-week bar row
  (Mon-Sun buckets) plus the **best day of the month** with its completion
  rate.
- **Export All (CSV)**: a second button on Report downloads the *entire*
  completion history (sorted by date) as one CSV, alongside the
  per-month export.
